...
export default function DashboardHome() {
  ...
  // Listen for global search events from the top bar
  useEffect(() => {
    const handler = (e: Event) => {
      const query = (e as CustomEvent<{ query: string }>).detail?.query || ''
      setSearchQuery(query)
    }
    window.addEventListener('classroom-search', handler)
    return () => window.removeEventListener('classroom-search', handler)
  }, [])

  if (loading) return <LoadingSpinner />

  const filterCourses = (list: ClassroomWithColor[]) => {
  ...
  const enrolledCourses = filterCourses(
    courses?.filter((c: ClassroomWithColor) => c.instructor_id !== user?.id) || []
  )
  const teachingCourses = filterCourses(
    courses?.filter((c: ClassroomWithColor) => c.instructor_id === user?.id) || []
  )
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-6 py-8">
        ...
        {searchQuery && (
          <p className="text-xs text-[#6B7280] mb-4">
            Showing results for "<span className="font-medium text-[#111827]">{searchQuery}</span>"
          </p>
        )}
          ...
          <div className="bg-white border border-dashed border-[#D1D5DB] rounded-xl p-8 text-center mb-12">
            <p className="text-[#6B7280] text-sm">
              {searchQuery
                ...
                : 'No enrolled courses yet. Join a class using a class code!'}
            </p>
          </div>
        )}
        ...
        {teachingCourses.length === 0 && (
          <div className="bg-white border border-dashed border-[#D1D5DB] rounded-xl p-8 text-center">
            <p className="text-[#6B7280] text-sm">
              {searchQuery
                ? `No teaching courses match "${searchQuery}".`

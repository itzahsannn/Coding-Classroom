          ...
          {!selectedStudentData ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-[#F8F9FA]">
                      ...
                      {studentFiles.map(f => (
                        <div key={f.path} onClick={() => handleDownload(f)} className="flex items-center gap-3 p-3 bg-white border border-[#DADCE0] rounded-lg cursor-pointer hover:border-[#1967D2] hover:shadow-sm transition-all group">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-[#3C4043] truncate group-hover:text-[#1967D2]">
                              {f.name.includes('_') ? f.name.substring(f.name.indexOf('_') + 1) : f.name}
                            </p>
                ...
                {/* AI Feedback */}
                {selectedStudentData.submission?.llm_feedback && (
                  <div className="mt-8">

...
export const ClassworkTab = ({ isTeacher, classroomId }: { isTeacher: boolean; classroomId?: string }) => {
  const navigate = useNavigate()
  const { data: assignments, loading } = useFetch(
    () => api.getAllAssignments(classroomId!), [classroomId]
  )
    ...
    <div className="max-w-4xl mx-auto px-6 py-8">
      {isTeacher && (
        <div className="mb-6 border-b border-[#E0E0E0] pb-6">
          <button
            onClick={() => navigate(`/course/${classroomId}/assignment/new`)}
      ...
      <div className="space-y-4">
        {assignments?.map((asn: Assignment) => (
          <div
            key={asn.id}
            onClick={() => navigate(`/course/${classroomId}/assignment/${asn.id}`)}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#5F6368] mt-0.5">
                  {asn.created_at ? new Date(asn.created_at).toLocaleDateString() : ''}
                </p>
              </div>
            </div>
            <div className="mt-3 md:mt-0 md:ml-4 text-right">
              <span className="text-xs font-medium text-[#5F6368]">
                {asn.deadline ? `Due ${new Date(asn.deadline).toLocaleDateString()}` : 'No due date'}
              </span>
            </div>
          </div>
        ))}
